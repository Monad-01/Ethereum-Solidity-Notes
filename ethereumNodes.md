...
5. Miner finds a valid block by solving the PoW puzzle (or PoS) and adds the new block to the exisiting blockchain. The number of transactions in the block depends on the block gas limit.
6. The miner node broadcasts the new block to all other nodes. All nodes execute the trasnactions from the new block, but only the miner gets paid. (block reward + transactions fee - gas).